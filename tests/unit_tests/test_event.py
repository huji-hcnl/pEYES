...
class TestEvent(unittest.TestCase):
    ...
    def test_init(self):
        t = np.arange(20)
        x = np.sin(np.pi * self._t / 10)
        y = np.cos(np.pi * self._t / 10)
        ...
        f1 = FixationEvent(t=self._t)
        f2 = FixationEvent(t=self._t, x=None, y=None)
        f3 = FixationEvent(t=self._t, x=x, y=y, pixel_size=self._PS, viewer_distance=self._VD)
