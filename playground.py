...
#########
events = pEYES.events.from_labels(
    labels=detection_results["ENGBERT"][0],
    t=trial1[pEYES.constants.T].values,
    x=trial1[pEYES.constants.X].values,
    y=trial1[pEYES.constants.Y].values,
...
labels = pEYES.events.to_labels(events, sampling_rate=500)

#########
ra_events = pEYES.events.from_labels(
    labels=trial1["RA"],
    t=trial1[pEYES.constants.T].values,
    x=trial1[pEYES.constants.X].values,
    y=trial1[pEYES.constants.Y].values,
...
#########
matches = pEYES.events.match(events, ra_events, "onset", allow_xmatch=False, max_onset_difference=200)
