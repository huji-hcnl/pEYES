...
def event_summary(
    ...
    event_labels = np.vectorize(parse_label)(inlier_features.index)
    ...
    for r, (metric, measure) in enumerate(subplots.items()):
        if r == 0:
            ...
            for evnt in event_labels:
                # TODO: keep placeholders for labels with no events?
                fig.add_trace(
                    row=r+1, col=1,
                    trace=go.Violin(
                        name=evnt, legendgroup=evnt, scalegroup=evnt, x0=evnt,
                        y=inlier_features.loc[evnt, metric],
                        fillcolor=inlier_color, opacity=opacity, showlegend=False, spanmode='hard',
                ...
                if show_outliers:
                    fig.add_trace(
                        ...
                        trace=go.Violin(
                            ...
                            fillcolor=outlier_color, opacity=opacity, showlegend=False, spanmode='hard',
                            box_visible=True, meanline_visible=True, line_color='black',
                            side='negative',
                        )
                    )
