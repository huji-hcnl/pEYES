        ...
        if metric == cnst.CENTER_PIXEL_STR:
            ...
            inlier_trace = go.Scatter(
                ...
                marker=dict(
                    ...
                    color=kwargs.get(
                        "inlier_color", f"rgb{vis_utils.get_label_colormap(None)[EventLabelEnum.FIXATION]}"
                    )
                ),
            )
            if show_outliers:
                ...
                outlier_trace = go.Scatter(
                    ...
                    marker=dict(
                        size=kwargs.get("marker_size", 5),
                        color=kwargs.get(
                            "outlier_color", f"rgb{vis_utils.get_label_colormap(None)[EventLabelEnum.FIXATION]}"
                        ),
                        opacity=kwargs.get("outlier_opacity", 0.5)
                    ),
                )
