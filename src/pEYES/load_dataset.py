...
def load_dataset(dataset_name: str, directory: str = None, save: bool = False) -> pd.DataFrame:
    """
    ...
    :param dataset_name: str; the name of the dataset to load, one of: "Lund2013", "IRF", "HFC", "GazeCom"
    ...
    if dataset_name_lower == "lund2013":
        return Lund2013DatasetLoader.load(directory, save)
    if dataset_name_lower == "irf":
        return IRFDatasetLoader.load(directory, save)
    if dataset_name_lower == "hfc":
        return HFCDatasetLoader.load(directory, save)
    if dataset_name_lower == "gazecom":
        return GazeComDatasetLoader.load(directory, save)
